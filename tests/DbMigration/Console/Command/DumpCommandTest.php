...
class DumpCommandTest extends AbstractTestCase
{
    ...
    protected function setup(): void
    {
        ...
        $this->readyObject($this->schema, (new Table('table1', [
            ...
            new Column('name', Type::getType('string'), ['default' => '', 'length' => 255]),
        ], [
    ...
    function integration()
    {
        ...
        $this->assertEquals(['table1'], array_values(array_map(fn($object) => $object->getName(), $schema->getTables())));
        $this->assertEquals(['view1'], array_values(array_map(fn($object) => $object->getName(), $schema->getViews())));
        $this->assertEquals(['trigger1'], array_values(array_map(fn($object) => $object->getName(), $schema->getTriggers())));
