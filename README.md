...
`スキーマ => sql`, `sql => スキーマ` は doctrine の機能で簡単に実現できますが、 `sql <=> スキーマ` で差分を取るのは多くの場合不可能で、一時スキーマが必要になります。
...
- hoge.utf8.csv: BOM の扱いはファイルタイプに任せます（仮定も推測もしません）
...
Options:
      --pre-suffix=PRE-SUFFIX             Specify pre-migration suffix
      --disable-constraint                Disable constraint (e.g. foreign key, unique, etc)
      --maintain-type|--no-maintain-type  Maintain type as much as possible.
  -d, --directory[=DIRECTORY]             Specify separative directory name.
  -m, --migration[=MIGRATION]             Specify migration directory.
...
事前に当てる migration ファイルのサフィックスを指定します。

migration ファイルはデフォルトではスキーマ適用後（DDL,DML 適用後）に当てられます。
これは新カラムにデフォルト値を入れたりデータ変換を考慮してだったんですが、スキーマ適用前に当てたい migration も存在します。

典型的には「互換性のない型の変更」です。
例えば「VARCHAR を JSON に変更する」等はスキーマ変更前に `JSON_ARRAY(column_name)` などで値を変換しないとスキーマ適用自体がコケてしまいます（`Invalid JSON text: "Invalid value."` 等）。

このような時、このオプションを指定するとそのサフィックスが付いた migration ファイルはスキーマ適用前に実行されるようになります。

- e.g. `--pre-suffix=-pre`
  - hoge-pre.sql // これはスキーマ適用前に実行される
  - hoge.sql     // これはスキーマ適用後に実行される（デフォルトの動作）
